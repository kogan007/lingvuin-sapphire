...
import { ButtonTypes, ButtonStyles } from './pagination.i';
import type { ButtonsTypes, PaginationOptions } from './pagination.i';
const defaultEmojis = {
	first: '⬅️',
	previous: '◀️',
	next: '▶️',
...
export const pagination = async (options: PaginationOptions) => {
	...
	let rows: any = [];
	actionRows && rows.push(actionRows[0]);
	...
	if (type === 'interaction' && channel) {
		if (goBack) {
			await interaction!
				//@ts-expect-error
				.deferUpdate({ ephemeral: ephemeralMessage })
				.catch(() => ({}));
		} else {
	...
	const collectorOptions = (filter?: any): any => {
	...
	if (pageTravel) {
		...
		collectorModal.on('collect', async (btnInteraction: any) => {
			// Show modal
			const modal = new ModalBuilder().setCustomId('choose_page_modal').setTitle('Choose Page');

			const inputPageNumber = new TextInputBuilder().setCustomId('page_number').setLabel('Enter Page Number').setStyle(TextInputStyle.Short);
			...
			await btnInteraction
				...
				.then(async (i: any) => {
					...
					await i.update({
						embeds: [changeFooter()],
						components: [actionRows![currentPage], ...components()],
	...
	collector.on('collect', async (btnInteraction: ButtonInteraction) => {
		...
		await btnInteraction.update({
			embeds: [changeFooter()],
			components: [actionRows![currentPage - 1], ...components()]
		});
	});
		...
		if (type === 'message') {
			...
			if (deleteAtEnd) {
				interaction!.deleteReply().catch((err) => {
					if (err.code === 50027) {
						console.log(
							`Webhook token has expired : ${
								client ? "trying to delete embed from it's id..." : 'no client found in option, cannot delete embed'
							}`
						);
						if (client) {
							client.channels
